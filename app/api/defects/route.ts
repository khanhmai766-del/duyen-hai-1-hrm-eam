...
export async function POST(req: NextRequest) {
  return handle(async () => {
    ...
    const defect = await prisma.$transaction(async (tx) => {
      const allocation = await allocateDefectRequestNumber(tx, requestYear, requestType, sheetTarget);
      ...
      if (allocation.reusedCancelledDefectId) {
        await tx.defect.update({ where: { id: allocation.reusedCancelledDefectId }, data: { requestNumberReusedById: created.id } });
      }
      await enqueueDefectSyncEvent(tx, {
        defect: created,
        eventType: "CREATE",
        extra: allocation.reusedCancelledDefectId ? { replacesCancelledDefectId: allocation.reusedCancelledDefectId } : undefined,
      });
      return created;
    });
    await audit(
