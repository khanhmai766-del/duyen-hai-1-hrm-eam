...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  return handle(async () => {
    ...
    if (user.id !== authorId && !(await hasPermissionLevel(user, "forum-moderate", ["manage", "full"]))) return fail("Bạn không có quyền sửa phản hồi này", 403);
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  return handle(async () => {
    ...
    const authorId = await getReplyAuthor(params.id);
    if (!authorId) return fail("Không tìm thấy phản hồi Forum", 404);
    if (user.id !== authorId && !(await hasPermissionLevel(user, "forum-moderate", ["manage", "full"]))) return fail("Bạn không có quyền gỡ phản hồi này", 403);
