...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  return permitHandle(async () => {
    ...
    const row = await prisma.$transaction(async tx => {
      ...
      const data = parsePermit(await resolvePermitIdentities(tx, { ...body, format: body.format ?? before.format }, user, before), status);
      ...
      const saved = await tx.workPermit.updateMany({ where: { id: params.id, version: before.version }, data: { ...data, status, version: { increment: 1 } } });
