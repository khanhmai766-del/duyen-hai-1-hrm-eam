...
export async function GET(req: Request) {
  return permitHandle(async () => {
    ...
    if (new URL(req.url).searchParams.get("summary") === "1") {
      const rows = await prisma.$queryRawUnsafe<Array<{ company: string; total: bigint; commanders: bigint; active: bigint }>>(`
        SELECT c.company, COUNT(p.id) AS total,
      ...
      return ok(rows.map(row => ({ company: row.company, total: Number(row.total), commanders: Number(row.commanders), active: Number(row.active) })), { canWrite: (await permitCapabilities(user)).canIssue });
    }
...
export async function POST(req: Request) {
  return permitHandle(async () => {
    ...
    const name = permitText(await permitBody(req), "name");
    ...
    const row = await prisma.workPermitCompany.create({ data: { name } });
    await audit(user.id, "CREATE_WORK_PERMIT_COMPANY", "WorkPermitCompany", row.id, `Thêm đơn vị nhà thầu "${name}"`);
    return ok(row);
  });
}
