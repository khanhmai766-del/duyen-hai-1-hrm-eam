...
import { ArrowRight, Check, CheckCircle2, ChevronLeft, ChevronRight, CircleAlert, ClipboardList, Download, ExternalLink, FileText, Filter, Plus, RefreshCw, RotateCcw, Search, ShieldCheck, SlidersHorizontal, UsersRound, Wrench, Zap, Clock } from "lucide-react";
...
export default function WorkPermitsPage() {
  ...
  return <div className="mx-auto max-w-[1600px] space-y-4 pb-24 font-sans text-[13px] md:pb-28">
    <header className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
      ...
      <div className="flex flex-wrap gap-2"><Button variant="outline" size="sm" className="h-9 text-xs" onClick={() => setDirectory(true)}><UsersRound />Nhân sự nhà thầu</Button><Button variant="outline" size="sm" className="h-9 text-xs" onClick={() => setExportOpen(true)} disabled={safetyTab || !meta || exporting.isPending || query.isError}><Download />{exporting.isPending ? "Đang xuất…" : "Xuất Excel"}</Button>{!safetyTab && meta?.canIssue && <Button size="sm" className="h-9 text-xs" onClick={() => setEditor("new")}><Plus />Ghi cấp phiếu</Button>}</div>
    </header>
...
function PermitEditor({ initial, kind, onClose, onSaved }: { initial?: PermitRow; kind: PermitKind; onClose: () => void; onSaved: (row: PermitRow) => void }) {
  ...
  return <Dialog open onOpenChange={v => { if (!v && !busy) onClose(); }}><DialogContent className="left-0 top-0 flex h-[100dvh] max-h-[100dvh] w-full max-w-full translate-x-0 translate-y-0 flex-col gap-0 overflow-hidden border-0 p-0 font-sans text-[13px] sm:left-[50%] sm:top-[50%] sm:h-[min(92dvh,900px)] sm:max-h-[900px] sm:w-[min(96vw,1180px)] sm:max-w-[1180px] sm:translate-x-[-50%] sm:translate-y-[-50%] sm:rounded-2xl sm:border">
    <div className="shrink-0 border-b border-border bg-background px-4 py-4 pr-14 sm:px-6 sm:py-5 sm:pr-14"><div className="flex flex-wrap items-start justify-between gap-3"><div><DialogTitle className="text-lg tracking-[-0.01em] sm:text-xl">{initial ? `Cập nhật PCT ${formatPermitNumber(initial)}` : "Ghi cấp phiếu công tác"}</DialogTitle><DialogDescription className="mt-1 max-w-2xl text-[13px] leading-5">Nhập theo phiếu thực tế. Các trường bắt buộc thay đổi theo trạng thái và loại đơn vị.</DialogDescription></div><div className="flex items-center gap-2"><PermitFormat permit={form} /><Status value={status} /></div></div></div>
      ...
      <fieldset disabled={busy} className="flex min-h-0 min-w-0 flex-1 flex-col"><datalist id={namesId}>{users.data?.data.map(u => <option key={u.id} value={u.name} />)}</datalist>
        ...
        <div className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[minmax(0,1fr)_290px]">
          <main className="min-h-0 min-w-0 overflow-y-auto bg-background px-4 py-5 sm:px-6 sm:py-6">
            ...
            {activeStep === "info" && <section id="permit-step-info" tabIndex={-1} aria-labelledby="permit-step-info-title" className="mx-auto max-w-3xl space-y-5 outline-none"><div><p className="text-[11px] font-semibold uppercase tracking-wider text-blue-700">Bước {activeStepIndex + 1}/{steps.length}</p><h2 id="permit-step-info-title" className="mt-1 text-lg font-bold tracking-tight text-foreground">Thông tin phiếu</h2><p className="mt-1 text-[13px] text-muted-foreground">Xác định loại phiếu, số sổ, thiết bị và nội dung công việc.</p></div>
