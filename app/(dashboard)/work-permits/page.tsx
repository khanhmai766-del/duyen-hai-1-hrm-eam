...
export default function WorkPermitsPage() {
  ...
  return <div className="mx-auto max-w-[1600px] space-y-4 font-sans text-[13px]">
    ...
    {bookTab && meta?.canIssue && Boolean(kindReservations.length) && <section className="rounded-xl border border-amber-200 bg-amber-50/70 p-3 sm:p-4"><h2 className="flex items-center gap-2 text-sm font-bold text-amber-950"><Clock size={16} />Số đã lấy, chưa lưu phiếu · {PERMIT_KINDS[kind]}</h2><p className="mt-1 text-xs text-amber-800">Số vẫn được giữ khi đóng biểu mẫu. Bấm Tiếp tục cấp phiếu để hoàn tất; chọn nhầm loại phiếu (nội bộ/nhà thầu) thì đổi ngay trong biểu mẫu, vẫn giữ nguyên số. Chỉ hiển thị lượt giữ số của sổ đang xem.</p><div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{kindReservations.map(item => <div key={item.id} className="rounded-lg border border-amber-200 bg-white p-3"><p className="font-semibold text-slate-900">{formatPermitNumber(item)} · {PERMIT_KINDS[item.kind as PermitKind]}</p><p className="mt-1 text-xs text-slate-600">{item.teamType === "INTERNAL" ? "Nội bộ · PCT điện tử" : "Nhà thầu · PCT giấy"} · {item.ownerName || "Người cấp"}</p><div className="mt-3 flex gap-2"><Button size="sm" className="h-8" onClick={() => { setKind(item.kind as PermitKind); setEditor(null); setNewPermit({ kind: item.kind as PermitKind, teamType: item.teamType, reservation: item }); }}>Tiếp tục cấp phiếu</Button>{pageSession?.user?.role === "ADMIN" && <Button size="sm" variant="outline" className="h-8" title="Chỉ quản trị: dùng khi lấy nhầm sổ Cơ/Điện" onClick={() => { setCancelReservationTarget(item); setCancelReservationReason(""); }}>Hủy lượt lấy</Button>}</div></div>)}</div></section>}
        ...
        <Table className="min-w-[1080px] table-fixed" wrapperClassName={cn("hidden md:block", TABLE_SCROLLER)}>
          ...
          <TableBody>{rows.map((r, index) => {
            ...
            return <TableRow key={r.id} tabIndex={0} aria-label={`Xem phiếu ${formatPermitNumber(r)}`} onClick={e => openRow(e, r.id)} onKeyDown={e => openRowByKey(e, r.id)} className={cn(rowBackground({ index }), ROW_HOVER, "cursor-pointer focus-visible:bg-sky-50 focus-visible:outline-none")}>
              <TableCell className={cn(TD_ROW, "py-3 text-center")}><div className="flex flex-col items-center gap-1"><WorkTypeBadge value={r.workType} /><PermitFormat permit={r} /></div></TableCell>
              ...
              <TableCell className={cn(TD_ROW, "py-3 text-center")}><div className="flex flex-col items-center"><Status value={r.status} />{r.teamType === "CONTRACTOR" && <PermitProgress value={r.progress} />}</div></TableCell>
            </TableRow>;
          })}</TableBody>
        </Table>
      </>}
