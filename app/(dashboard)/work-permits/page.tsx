...
export default function WorkPermitsPage() {
  ...
  return <div className="mx-auto max-w-[1600px] space-y-4 font-sans text-[13px]">
    <header className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
      ...
      <div className="flex flex-wrap gap-2"><Button variant="outline" size="sm" className="h-9 text-xs" onClick={() => setDirectory(true)}><UsersRound />Nhân sự nhà thầu</Button>{pageSession?.user?.role === "ADMIN" && <Button variant="outline" size="sm" className="h-9 text-xs" onClick={() => setBaselineOpen(true)}>Mốc sổ giấy</Button>}<Button variant="outline" size="sm" className="h-9 text-xs" onClick={() => setExportOpen(true)} disabled={safetyTab || !meta || exporting.isPending || query.isError}><Download />{exporting.isPending ? "Đang xuất…" : "Xuất Excel"}</Button>{!safetyTab && meta?.canIssue && <><Button size="sm" className="h-9 bg-blue-800 text-xs hover:bg-blue-900" onClick={() => startNewPermit("INTERNAL")}><Building2 />Cấp phiếu nội bộ</Button><Button size="sm" className="h-9 bg-cyan-700 text-xs hover:bg-cyan-800" onClick={() => startNewPermit("CONTRACTOR")}><HardHat />Cấp phiếu nhà thầu</Button></>}</div>
    </header>
    ...
    {!safetyTab && meta?.canIssue && Boolean(reservations.data?.data.length) && <section className="rounded-xl border border-amber-200 bg-amber-50/70 p-3 sm:p-4"><h2 className="flex items-center gap-2 text-sm font-bold text-amber-950"><Clock size={16} />Số đã lấy, chưa lưu phiếu</h2><p className="mt-1 text-xs text-amber-800">Số vẫn được giữ khi đóng biểu mẫu. Tiếp tục cấp phiếu hoặc hủy lượt lấy số có lý do.</p><div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{reservations.data?.data.map(item => <div key={item.id} className="rounded-lg border border-amber-200 bg-white p-3"><p className="font-semibold text-slate-900">{formatPermitNumber(item)} · {PERMIT_KINDS[item.kind as PermitKind]}</p><p className="mt-1 text-xs text-slate-600">{item.teamType === "INTERNAL" ? "Nội bộ · PCT điện tử" : "Nhà thầu · PCT giấy"} · {item.ownerName || "Người cấp"}</p><div className="mt-3 flex gap-2"><Button size="sm" className="h-8" onClick={() => { setKind(item.kind as PermitKind); setEditor(null); setNewPermit({ kind: item.kind as PermitKind, teamType: item.teamType, reservation: item }); }}>Tiếp tục cấp phiếu</Button><Button size="sm" variant="outline" className="h-8" onClick={() => { setCancelReservationTarget(item); setCancelReservationReason(""); }}>Hủy lượt lấy</Button></div></div>)}</div></section>}
        ...
        <div className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[minmax(0,1fr)_290px]">
          <main className="min-h-0 min-w-0 overflow-y-auto bg-background px-4 py-5 sm:px-6 sm:py-6">
            ...
            {activeStep === "status" && <section id="permit-step-status" tabIndex={-1} aria-labelledby="permit-step-status-title" className="mx-auto max-w-3xl space-y-5 outline-none">
              ...
              <div className="grid gap-4 rounded-xl border border-border p-4 md:grid-cols-2 sm:p-5">
