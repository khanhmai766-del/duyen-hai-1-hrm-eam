...
import { useCancelDraftWorkPermit, useWorkPermits, useWorkPermit, useSaveWorkPermit, useExportWorkPermits, usePermitNumberSuggestion, useUploadPermitAttachment, useDeletePermitAttachment } from "@/hooks/useWorkPermits";
...
import { DEFAULT_PERMIT_MANAGING_UNIT, DEFAULT_PERMIT_PLANT, permitEquipmentItems, type PermitAttachment } from "@/lib/work-permit-source-fields";
...
function PermitEditor({ initial, kind, presetTeamType, onClose, onSaved }: { initial?: PermitRow; kind: PermitKind; presetTeamType?: PermitInput["teamType"]; onClose: () => void; onSaved: (row: PermitRow) => void }) {
  ...
  const uploadAttachment = useUploadPermitAttachment();
  const deleteAttachment = useDeletePermitAttachment();
  const [pendingFiles, setPendingFiles] = useState<File[]>([]);
  const [attachments, setAttachments] = useState<PermitAttachment[]>(initial?.attachments ?? []);
  const [workingId, setWorkingId] = useState(initial?.id);
  const [workingVersion, setWorkingVersion] = useState(initial?.version);
  ...
  function changeFormat(format: PermitFormatValue, teamType = form.teamType) {
    ...
    if (format !== "PAPER" && form.kind === "MECHANICAL") {
      if (attachments.length) { toast.error("Hãy xóa tệp đính kèm trước khi đổi hình thức phiếu."); return; }
      if (pendingFiles.length && !window.confirm("Đổi hình thức phiếu sẽ bỏ các tệp đang chờ tải. Tiếp tục?")) return;
      setPendingFiles([]);
    }
  ...
  function changeKind(kind: PermitKind) {
    ...
    if (kind !== "MECHANICAL" && form.kind === "MECHANICAL" && paper) {
      if (attachments.length) { toast.error("Hãy xóa tệp đính kèm trước khi đổi loại PCT."); return; }
      if (pendingFiles.length && !window.confirm("Đổi loại PCT sẽ bỏ các tệp đang chờ tải. Tiếp tục?")) return;
      setPendingFiles([]);
    }
  ...
  if (paper && form.kind === "MECHANICAL") {
    ...
    requireValue(Boolean(form.plantName?.trim()), "info", "Nhà máy");
    requireValue((form.equipmentItems ?? []).every(item => Boolean(item.code.trim() || item.name.trim())), "info", "Mã hoặc tên cho từng thiết bị");
  }
  ...
  async function cancelDraftNow() {
    if (!workingId || (initial && initial.status !== "DRAFT") || !window.confirm("Hủy PCT nháp này? Thao tác không yêu cầu điền CHTT hay các thông tin còn thiếu và phiếu sẽ bị khóa chỉnh sửa.")) return;
    try { const row = await cancelDraft.mutateAsync({ id: workingId, version: workingVersion! }); toast.success("Đã hủy PCT nháp"); onSaved(row); }
    catch (error) { toast.error(error instanceof Error ? error.message : "Không thể hủy PCT nháp"); }
  }
  async function removeAttachment(attachmentId: string) {
    if (!workingId || !workingVersion || !window.confirm("Xóa tệp đính kèm này khỏi phiếu?")) return;
    try {
      const result = await deleteAttachment.mutateAsync({ id: workingId, attachmentId, version: workingVersion });
      setWorkingVersion(result.version);
      setAttachments(items => items.filter(item => item.id !== attachmentId));
      toast.success("Đã xóa tệp đính kèm");
    } catch (error) { toast.error(error instanceof Error ? error.message : "Không thể xóa tệp"); }
  }
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      let id = workingId;
      const issueAfterFiles = status === "ISSUED" && pendingFiles.length > 0 && (!initial || initial.status === "DRAFT");
      let row = await save.mutateAsync({ id, body: { ...body, ...(issueAfterFiles ? { status: "DRAFT", issuedAt: null, authorizedAt: null, closedAt: null } : {}), version } });
      id = row.id; version = row.version;
      setWorkingId(id); setWorkingVersion(version);
      for (const file of [...pendingFiles]) {
        const uploaded = await uploadAttachment.mutateAsync({ id, version, file });
        version = uploaded.version;
        setWorkingVersion(version);
        setAttachments(items => [...items, uploaded.attachment]);
        setPendingFiles(files => files.filter(item => item !== file));
      }
      if (issueAfterFiles) {
        row = await save.mutateAsync({ id, body: { ...body, version } });
        version = row.version;
        setWorkingVersion(version);
      }
      toast.success(initial ? "Đã cập nhật sổ PCT" : "Đã ghi phiếu vào sổ");
      onSaved({ ...row, version });
    }
    catch (error) {
  ...
  const busy = save.isPending || cancelDraft.isPending || uploadAttachment.isPending || deleteAttachment.isPending || savingFiles;
          ...
          <main className="min-h-0 min-w-0 overflow-y-auto bg-background px-4 py-5 sm:px-6 sm:py-6">
            ...
            {activeStep === "info" && <section id="permit-step-info" tabIndex={-1} aria-labelledby="permit-step-info-title" className="mx-auto max-w-3xl space-y-5 outline-none"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h2 id="permit-step-info-title" className="text-lg font-bold tracking-tight text-foreground">Thông tin phiếu</h2><p className="mt-1 text-[13px] text-muted-foreground">Xác định loại phiếu, số sổ, thiết bị và nội dung công việc.</p></div><p className="shrink-0 whitespace-nowrap text-[11px] font-semibold uppercase tracking-wider text-blue-700 dark:text-blue-300">Bước {activeStepIndex + 1}/{steps.length}</p></div>
              ...
              <div className="grid gap-4 md:grid-cols-3"><label className="space-y-1.5 text-sm"><span className="font-medium">Loại PCT *</span><select className={control} value={form.kind} disabled={!numberEditable || Boolean(presetTeamType)} onChange={e => changeKind(e.target.value as PermitKind)}>{Object.entries(PERMIT_KINDS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}</select>{presetTeamType && <p className="text-xs text-muted-foreground">Đã chọn trước khi mở biểu mẫu.</p>}</label><div className="space-y-1.5 text-sm"><label className="block space-y-1.5"><span className="font-medium">Số PCT *</span><input ref={numberInputRef} className={control} aria-invalid={!form.number.trim()} maxLength={80} value={form.number} disabled={!numberEditable} onChange={e => set("number", e.target.value)} /></label><p className="break-words text-xs text-muted-foreground">{form.number.trim() ? `Số đầy đủ: ${formatPermitNumber(form)}` : "Chỉ cần nhập số; hệ thống tự ghép năm và /VH1-NĐDH."}</p>{numberEditable && <div className="flex min-h-8 flex-wrap items-center gap-x-2 gap-y-1 rounded-lg bg-sky-50 px-2.5 py-2 text-xs text-sky-950" aria-live="polite">{numberSuggestion.isPending ? <span>Đang tìm số cao nhất trong năm…</span> : numberSuggestion.isError ? <span>Chưa lấy được gợi ý số PCT.</span> : <><span>Số cao nhất năm {form.year}: <strong className="tabular-nums">{numberSuggestion.data?.data.highest ?? "chưa có"}</strong></span>{numberSuggestion.data?.data.suggested && <Button type="button" variant="ghost" size="sm" className="h-7 px-2 text-xs text-blue-700 hover:text-blue-800" onClick={() => { set("number", numberSuggestion.data!.data.suggested!); numberInputRef.current?.focus(); }}>Điền số {numberSuggestion.data.data.suggested}</Button>}</>}</div>}</div><label className="space-y-1.5 text-sm"><span className="font-medium">Năm cấp số *</span><input className={control} aria-invalid={!Number.isInteger(form.year) || form.year < 2000 || form.year > 2100} type="number" min={2000} max={2100} value={form.year} disabled={!numberEditable} onChange={e => set("year", Number(e.target.value))} /><p className="text-xs text-muted-foreground">Dãy số riêng cho từng loại PCT và từng năm.</p></label></div>
                             {paper && form.kind === "MECHANICAL" && <MechanicalPaperInfo form={form} issued={issued} onChange={set} onPickSyc={() => setPickSyc(true)} pendingFiles={pendingFiles} onPendingFilesChange={setPendingFiles} attachments={attachments} onDeleteAttachment={id => void removeAttachment(id)} attachmentBusy={busy} permitId={workingId} />}
...
const PERMIT_DETAIL_GROUPS: Array<{ title: string; keys: string[] }> = [
  { title: "Công việc", keys: ["managingUnit", "plantName", "equipmentItems", "location", "workScope", "unit", "position", "workDate", "workType", "kind", "disciplines", "repairRequestNumber", "registrationNumber"] },
...
const PERMIT_DETAIL_WIDE = new Set(["equipmentItems", "workScope", "result", "note", "statusReason", "members"]);
...
function PermitDetail({ id, canIssue: listCanIssue, canExecute: listCanExecute, onClose, onEdit }: { id: string; canIssue: boolean; canExecute: boolean; onClose: () => void; onEdit: (r: PermitRow) => void }) {
  ...
  return <Dialog open onOpenChange={v => { if (!v && !cancelDraft.isPending) onClose(); }}><DialogContent className="max-w-3xl">
    ...
    {query.isError ? <p role="alert">{query.error.message}</p> : !row ? <p>Đang tải phiếu…</p> : <div className="space-y-4">
      ...
      {paper && row.attachments.length > 0 && <section className="rounded-lg border border-border p-3"><h4 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Tài liệu đính kèm ({row.attachments.length})</h4><ul className="mt-2 space-y-1.5">{row.attachments.map(item => <li key={item.id} className="flex items-center justify-between gap-3 text-sm"><a className="min-w-0 truncate text-blue-700 underline-offset-2 hover:underline" href={`/api/work-permits/${row.id}/attachments/${item.id}`} target="_blank" rel="noopener noreferrer">{item.originalName}</a><span className="shrink-0 text-xs text-muted-foreground">{(item.bytes / 1024 / 1024).toFixed(1)} MB</span></li>)}</ul></section>}
