...
> **Ghi chú cho repo duyen-hai-1-hrm-eam** (bản gốc: [Leonxlnx/taste-skill](https://github.com/Leonxlnx/taste-skill) `skills/image-to-code-skill` @ `c184364`, MIT — xem `LICENSE.txt`).
> Khi áp dụng trong dự án này, các quy ước ở `AGENTS.md` được ưu tiên hơn nội dung skill bên dưới:
> - Giữ nguyên stack: Next.js App Router + Tailwind + shadcn/ui + TanStack Query. Không thêm thư viện UI, icon hay font mới nếu chưa kiểm tra `package.json` và chưa được người dùng đồng ý.
> - Toàn bộ chữ hiển thị là **tiếng Việt**; font phải hiển thị đủ dấu tiếng Việt. Dùng nhãn trong `lib/constants.ts`, so khớp chuỗi bằng `normalizeText()` của `lib/nav.ts`.
> - Đây là ứng dụng nội bộ nhà máy, dày dữ liệu (bảng, sổ theo dõi, form). Ưu tiên dễ đọc, mật độ vừa phải, dùng được trên điện thoại ca trực — bỏ qua các gợi ý chỉ hợp với landing page (hero, parallax, ảnh picsum, cookie banner, legal links…).
> - Ảnh tĩnh đi qua `next/image` theo `localPatterns` trong `next.config.mjs`; không thêm `remotePatterns`.
> - Trong repo này, "ảnh thiết kế" thường là ảnh chụp/ảnh mẫu người dùng gửi (sổ giấy, màn hình hệ thống cũ, mockup). Nếu không có công cụ sinh ảnh, bỏ bước tự sinh ảnh và phân tích trực tiếp ảnh được cung cấp.
