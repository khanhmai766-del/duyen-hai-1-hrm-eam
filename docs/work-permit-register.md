...
- Không tạo phiếu nháp mới. Người có quyền chọn sổ Cơ/Điện, mở luồng nhà thầu hoặc nội bộ rồi bấm **Lấy số PCT**. API khóa dãy số theo loại/năm và ghi lượt giữ ngay khi bấm; hai lượt bấm đồng thời nhận hai số khác nhau. Số/năm/loại được khóa sau khi lấy. Đóng biểu mẫu không trả số: mục **Số đã lấy, chưa lưu phiếu** cho tiếp tục hoặc hủy lượt giữ có lý do. Phiếu nháp cũ vẫn được xem và phải lấy số khi hoàn tất cấp phiếu.
- Số tự động đi sau số lớn nhất từng lấy/cấp và mốc sổ giấy, kể cả số đã hủy. **Số đã hủy bị bỏ luôn, không cấp lại** (từ 9/2026): ô Số PCT không nhập tay, chỉ lấy bằng nút **Lấy số PCT**. Phiếu giấy hủy vẫn lưu trong sổ ở trạng thái Đã hủy; PCT điện tử hủy thì NKVH đã lưu, tiện ích NKVH chỉ báo hủy về sổ. API lưu phiếu mới yêu cầu lượt giữ hợp lệ và chuyển sang Đã cấp trong cùng giao dịch. Mốc sổ giấy được ADMIN thiết lập riêng theo loại/năm, có lý do và lịch sử; chưa thiết lập mốc thì không cấp số mới.
...
- Bộ kiểm tra qua HTTP thật và PostgreSQL local bao gồm: từ chối CHTT không hợp lệ, mặc định người cấp theo tài khoản nhưng cho phép sửa tên thực tế, bảo vệ định danh CHTT, hình thức mặc định, nội bộ phiếu giấy, lưu lại lựa chọn và từ chối giá trị sai; cùng các tình huống đăng nhập, phân quyền, tạo trùng số, mở đồng thời Cơ/Điện cùng CHTT, đổi người giữa các lần, thời điểm tiếp giáp/giao nhau/qua ngày, khóa phiếu đang mở, giữ lịch sử khi đổi danh bạ, vòng đời phiếu nội bộ, chống cập nhật cùng phiên bản và xuất Excel.
...
Wizard 4 bước giữ nguyên; bước Thông tin gom các mục trên phiếu nguồn: đơn vị QLVH, nhà máy, đơn vị công tác, số ĐKCT, tổ máy, SYC, chuyên môn, địa điểm, nội dung, phạm vi và thời gian dự kiến. Đơn vị QLVH và nhà máy mặc định lần lượt là “Phân xưởng Vận hành 1” và “Duyên Hải 1”, có thể sửa. Nhóm Phân loại của phiếu giấy có ba lựa chọn Kế hoạch, Ngoài kế hoạch và Đột xuất; trường này độc lập với bộ KH/ĐX/SC cũ. Phiếu cũ chưa có phân loại mới để trống.
...
Phiếu công tác không quản lý hình ảnh hoặc tài liệu đính kèm. Không tạo bảng `WorkPermitAttachment`; môi trường đã từng tạo bảng này dùng `prisma/manual/remove-work-permit-attachments.sql` sau khi xử lý dữ liệu tệp hiện có.

Schema các trường nguồn cần `prisma/manual/add-work-permit-source-fields.sql` và `prisma/manual/add-work-permit-source-classification.sql` trên môi trường đã có sổ PCT. Phiếu cũ nhận mặc định hai đơn vị. Môi trường đã có cột thiết bị từ bản trước cần `prisma/manual/remove-work-permit-equipment-items.sql` để bỏ cột đó; việc này xóa dữ liệu thiết bị đã lưu trong cột.
