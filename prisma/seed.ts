...
async function main() {
    ...
    devices.push(
      await prisma.equipmentNode.upsert({
        ...
        update: {
          ...
          depth: 1,
          sort: 10_000 + i,
          attachedInfo: `Hệ thống: ${d.system}; Cương vị quản lý: ${d.managingPosition}`,
          deviceSynced: true,
        },
        create: {
          seq: d.code,
  ...
  await prisma.equipmentMaterial.createMany({
    data: [
      { deviceSeq: devices[4].seq, materialId: materials[0].id, quantity: 2, note: "Thay vòng bi bơm" },
      { deviceSeq: devices[5].seq, materialId: materials[0].id, quantity: 2, note: "Thay vòng bi bơm 1B" },
      { deviceSeq: devices[3].seq, materialId: materials[2].id, quantity: 1, note: "Thay gioăng FGD" },
      { deviceSeq: devices[12].seq, materialId: materials[1].id, quantity: 40, note: "Thay dầu tuabin" },
      { deviceSeq: devices[0].seq, materialId: materials[3].id, quantity: 2, note: "Thay điện cực ESP" },
    ],
  });
  console.log("✓ equipment material usage");

  // ---- Audit log sample ----
  await prisma.auditLog.createMany({
    data: [
      { userId: users[0].id, action: "CREATE_EQUIPMENT_NODE", entity: "EquipmentNode", entityId: devices[0].id, detail: "Thêm thiết bị ESP-S1-001" },
