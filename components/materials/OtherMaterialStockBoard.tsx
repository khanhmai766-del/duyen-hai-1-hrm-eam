...
import { ArrowDownToLine, ArrowUpFromLine, Boxes, CalendarDays, History, Loader2, MapPin, PackageCheck, Pencil, Search, UserRound } from "lucide-react";
...
export default function OtherMaterialStockBoard() {
  ...
  const materials = useMemo(() => {
    ...
    return (stock?.materials ?? [])
      .filter((row) =>
        ...
        && (stockFilter === "ALL" || row.quantity > 0)
        && (!q || `${row.code} ${row.erpCodes.join(" ")} ${row.name} ${row.category ?? ""} ${row.positions.join(" ")}`.toLocaleLowerCase("vi").includes(q))
      )
      .sort((a, b) => Number(b.category?.trim().toLocaleLowerCase("vi") === "chai khí") - Number(a.category?.trim().toLocaleLowerCase("vi") === "chai khí"));
  }, [category, search, stock?.materials, stockFilter]);
  ...
  return <div className="space-y-5">
    <div className="grid gap-2 sm:grid-cols-3">
      <button type="button" aria-pressed={stockFilter === "ALL"} onClick={() => setStockFilter("ALL")} className={`group flex min-h-[92px] items-center gap-3 rounded-xl border p-3.5 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-500 focus-visible:ring-offset-2 ${stockFilter === "ALL" ? "border-slate-900 bg-slate-900 text-white ring-2 ring-slate-300" : "border-slate-200 bg-white text-slate-900"}`}>
        <span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${stockFilter === "ALL" ? "bg-white/10 text-sky-300" : "bg-slate-100 text-slate-600"}`}><Boxes className="h-5 w-5" /></span>
        <span className="min-w-0"><span className={`block text-[11px] font-bold uppercase tracking-wider ${stockFilter === "ALL" ? "text-slate-300" : "text-slate-500"}`}>Tất cả vật tư</span><span className="mt-0.5 block text-2xl font-black leading-none">{stock?.materials.length ?? 0}</span></span>
      </button>
      <button type="button" aria-pressed={stockFilter === "IN_STOCK"} onClick={() => setStockFilter("IN_STOCK")} className={`group flex min-h-[92px] items-center gap-3 rounded-xl border p-3.5 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2 ${stockFilter === "IN_STOCK" ? "border-emerald-500 bg-emerald-600 text-white ring-2 ring-emerald-200" : "border-emerald-200 bg-emerald-50 text-emerald-950"}`}>
        <span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${stockFilter === "IN_STOCK" ? "bg-white/15 text-white" : "bg-emerald-100 text-emerald-700"}`}><PackageCheck className="h-5 w-5" /></span>
        <span className="min-w-0"><span className={`block text-[11px] font-bold uppercase tracking-wider ${stockFilter === "IN_STOCK" ? "text-emerald-50" : "text-emerald-700"}`}>Vật tư còn hàng</span><span className="mt-0.5 block text-2xl font-black leading-none">{stock?.materials.filter((row) => row.quantity > 0).length ?? 0}</span></span>
      </button>
      <button type="button" onClick={() => document.getElementById("other-material-history")?.scrollIntoView({ behavior: "smooth", block: "start" })} className="group flex min-h-[92px] items-center gap-3 rounded-xl border border-blue-200 bg-blue-50 p-3.5 text-left text-blue-950 shadow-sm transition hover:-translate-y-0.5 hover:border-blue-400 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2">
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-blue-100 text-blue-700"><History className="h-5 w-5" /></span>
        <span className="min-w-0"><span className="block text-[11px] font-bold uppercase tracking-wider text-blue-700">Lịch sử nhập / cấp / dùng</span><span className="mt-0.5 block text-2xl font-black leading-none">{stock?.movementPagination.total ?? 0}</span></span>
      </button>
    </div>

    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="flex flex-col gap-3 border-b border-slate-200 p-4 lg:flex-row lg:items-center lg:justify-between">
        <div><h2 className="font-bold text-slate-900">Hiện có tại phân xưởng</h2><p className="text-sm text-slate-500">{stockFilter === "IN_STOCK" ? "Đang hiển thị vật tư còn hàng." : "Đang hiển thị toàn bộ danh mục, gồm cả vật tư hết tồn."} “Chung” được cấp cho người nhận, vật tư có cương vị chỉ ghi nhận sử dụng.</p></div>
