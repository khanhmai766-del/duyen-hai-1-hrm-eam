...
type StockMaterial = {
  id: string; code: string; name: string; unit: string; category: string | null; machine: string;
...
type StockMovement = {
  id: string; type: "RECEIPT" | "ISSUE" | "USE"; quantity: number; stockBefore: number; stockAfter: number;
...
export default function OtherMaterialStockBoard() {
  ...
  const materials = useMemo(() => {
    ...
    return (stock?.materials ?? []).filter((row) =>
      ...
      && (!q || `${row.code} ${row.name} ${row.category ?? ""} ${row.positions.join(" ")}`.toLocaleLowerCase("vi").includes(q))
    );
  ...
  return <div className="space-y-5">
    <div className="grid gap-3 sm:grid-cols-3">
      ...
      <div className="rounded-2xl border border-blue-200 bg-blue-50 p-5 text-blue-950">
        <History className="mb-4 h-5 w-5 text-blue-600" /><p className="text-xs font-semibold uppercase tracking-wider text-blue-700">Lượt nhập / cấp / dùng</p><p className="mt-1 text-3xl font-black">{stock?.movements.length ?? 0}</p>
      </div>
    </div>
    ...
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="max-h-[620px] overflow-auto"><table className="w-full min-w-[1000px] text-sm"><thead className="sticky top-0 bg-slate-50 text-left text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Ngày</th><th className="px-4 py-3">Nghiệp vụ</th><th className="px-4 py-3">Vật tư</th><th className="px-4 py-3">Số lượng</th><th className="px-4 py-3">Người nhận / thực hiện</th><th className="px-4 py-3">Hiện có</th><th className="px-4 py-3">Ghi chú</th></tr></thead><tbody className="divide-y divide-slate-100">{(stock?.movements ?? []).map((row) => <tr key={row.id}><td className="px-4 py-3 whitespace-nowrap">{day(row.occurredAt)}</td><td className="px-4 py-3"><span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-bold ${row.type === "RECEIPT" ? "bg-emerald-50 text-emerald-700" : row.type === "ISSUE" ? "bg-amber-50 text-amber-700" : "bg-blue-50 text-blue-700"}`}>{row.type === "RECEIPT" ? <ArrowDownToLine className="h-3.5 w-3.5"/> : <ArrowUpFromLine className="h-3.5 w-3.5"/>}{movementLabel[row.type]}</span></td><td className="px-4 py-3"><b>{row.material.name}</b><small className="block text-slate-500">{row.material.code}</small></td><td className="px-4 py-3 font-bold">{row.type === "RECEIPT" ? "+" : "−"}{row.quantity} {row.material.unit}</td><td className="px-4 py-3">{row.receiverName || row.createdByName}<small className="block text-slate-500">{row.type === "ISSUE" ? `Người phát: ${row.issuerName}` : row.assignedPosition}</small></td><td className="px-4 py-3 font-semibold">{row.stockBefore} → {row.stockAfter}</td><td className="max-w-[260px] px-4 py-3 text-slate-600">{row.note || "—"}</td></tr>)}</tbody></table>{(stock?.movements.length ?? 0) === 0 && <div className="p-10 text-center text-slate-500">Không tìm thấy lịch sử phù hợp.</div>}</div>
