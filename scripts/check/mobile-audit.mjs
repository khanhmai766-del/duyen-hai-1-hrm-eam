 ...
 *
 *   node scripts/check/mobile-audit.mjs                                   # soát localhost:3000
 *   node scripts/check/mobile-audit.mjs --url https://duyenhai1.vn
 *   node scripts/check/mobile-audit.mjs --pages defects,materials         # chỉ vài trang
 *   node scripts/check/mobile-audit.mjs --devices se,s8                   # chỉ vài dòng máy
 *   node scripts/check/mobile-audit.mjs --save truoc.json                 # lưu mốc so sánh
 *   node scripts/check/mobile-audit.mjs --compare truoc.json              # so với mốc đã lưu
 *   node scripts/check/mobile-audit.mjs --no-shots                        # bỏ chụp ảnh cho nhanh
 *
 * Tài khoản lấy từ biến môi trường (KHÔNG ghi mật khẩu vào file này):
 *   AUDIT_EMAIL=admin@powerplant.vn AUDIT_PASSWORD=... node scripts/check/mobile-audit.mjs
 *
...
if (!PASSWORD) {
  console.error("Thiếu AUDIT_PASSWORD. Ví dụ:\n  AUDIT_PASSWORD='...' node scripts/check/mobile-audit.mjs");
  process.exit(1);
}
