 ...
 *
 * Chạy:   npx tsx scripts/check/demo-chemical-inventory.ts
 * Gỡ ra:  npx tsx scripts/check/demo-chemical-inventory.ts --xoa
 *
...
async function main() {
  ...
  const item = await prisma.chemicalInventoryItem.findUnique({ where: { code: "NH3_99" } });
  if (!item) throw new Error("Chưa có danh mục — chạy scripts/import/seed-chemical-inventory.ts trước.");
  ...
  console.log("  Gỡ dữ liệu mẫu:       npx tsx scripts/check/demo-chemical-inventory.ts --xoa\n");
}
