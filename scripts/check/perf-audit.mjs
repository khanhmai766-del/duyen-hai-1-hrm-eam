 ...
 *
 *   node scripts/check/perf-audit.mjs                          # đo localhost:3000
 *   node scripts/check/perf-audit.mjs --url https://duyenhai1.vn
 *   node scripts/check/perf-audit.mjs --save truoc.json        # lưu mốc so sánh
 *   node scripts/check/perf-audit.mjs --compare truoc.json     # so với mốc đã lưu
 *   node scripts/check/perf-audit.mjs --pages defects,reports   # chỉ đo vài trang
 *   node scripts/check/perf-audit.mjs --poll                   # kiểm tra polling khi tab bị ẩn
 *
 * Tài khoản lấy từ biến môi trường (KHÔNG ghi mật khẩu vào file này):
 *   AUDIT_EMAIL=admin@powerplant.vn AUDIT_PASSWORD=... node scripts/check/perf-audit.mjs
 *
...
if (!PASSWORD) {
  console.error("Thiếu AUDIT_PASSWORD. Ví dụ:\n  AUDIT_PASSWORD='...' node scripts/check/perf-audit.mjs");
  process.exit(1);
}
