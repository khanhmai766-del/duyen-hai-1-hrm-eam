...
async function migrateUsers() {
  const rows = await prisma.user.findMany({ select: { id: true, avatarUrl: true, signatureUrl: true } });
  for (const row of rows) {
    const avatarUrl = await migrateUrl(row.avatarUrl, "avatars", "avatar");
    const signatureUrl = await migrateUrl(row.signatureUrl, "signatures", "signature");
    if (avatarUrl !== row.avatarUrl || signatureUrl !== row.signatureUrl) {
      await prisma.user.update({ where: { id: row.id }, data: { avatarUrl, signatureUrl } });
      console.log(`Đã migrate User ${row.id}`);
    }
  }
}
