...
async function main() {
  ...
  try {
    ...
    expect(await request("/api/work-permits", "POST", { ...base, number: prefix + "-electric-safety", kind: "ELECTRICAL", safetyItems: [{ ...pair, sourceId: elec.data.id, forExecution: true }] }), 400, "PCT Điện không nhận biện pháp điền sẵn");
    const electrical = await request("/api/work-permits", "POST", { ...base, number: prefix + "-electric", kind: "ELECTRICAL", leaderName: "LĐCV điện", electricalSafetySupervisorName: "GSATĐ thử", safetyItems: [] }); expect(electrical, 200, "Cấp PCT Điện giấy tối giản"); permitIds.push(electrical.data.id);
